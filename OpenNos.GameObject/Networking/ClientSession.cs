...
namespace OpenNos.GameObject
{
    public class ClientSession
    {
        ...
        public async Task SendPacketAsync(string packet, byte priority = 10)
        {
            if (!IsDisposing)
            {
                await _client.SendPacketAsync(packet, priority).ConfigureAwait(false);
        ...
        {
            IEnumerable<Type> handlerTypes = !isWorldServer ? type.Assembly.GetTypes().Where(t => t.Name.Equals("NoS0575PacketHandler")) // shitty but it works NoS0575PacketHandler
        ...
        public void SendPacket(Func<string> generateFood)
        {
