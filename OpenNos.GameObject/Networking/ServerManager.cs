...
namespace OpenNos.GameObject.Networking
{
    public class ServerManager : BroadcastableBase
    {
        ...
        public void ChangeMapInstance(long characterId, Guid mapInstanceId, int? mapX = null, int? mapY = null, bool noAggroLoss = false)
        {
            ...
            if (session?.Character != null && !session.Character.IsChangingMapInstance)
            {
                ...
                try
                {
                    ...
                    session.SendPacket(session.Character.GenerateStp());
                    session.SendPacket(session.Character.GenerateStp2());
                    session.SendPacket(session.Character.GenerateCInfo());
                    session.SendPacket(session.Character.GenerateCMode());
                    session.SendPacket(session.Character.GenerateEq());
                    session.SendPacket(session.Character.GenerateMiniPet());
                    ...
                    if (session.CurrentMapInstance.Map.MapTypes.All(s => s.MapTypeId != (short)MapTypeEnum.Act52) /* == true && session.Character.Buff.Any(s => s.Card.CardId == 339 && session.Character.MapInstance.Map.MapId == 214)*/) //Act5.2 ehm warte  weist du was ... ok probiwer mal jetzt
                    {
                        session.Character.RemoveBuff(339); // probier mal jetzt ok 
                    }
                    ...
                    //   session.CurrentMapInstance.GenerateRsfn();
                    session.SendPacket("rsfm 0 0 4 12 0");
        ...
        public static ServerManager GetInstance { get; } = new ServerManager();
        ...
        public static double RandomNumberD(int minimum = 0,int maximum = 100)
        {
            lock(syncLock)
            {
