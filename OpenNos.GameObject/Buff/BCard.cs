                    ...
                    case BCardType.CardType.Buff:
                        {
                            ...
                            if (SubType == 1)
                            {
                                ...
                                {
                                    //if(SkillVNum != null && (buff.SkillVNum == 1187 || (buff.SkillVNum >= 1191 && buff.SkillVNum <= 1196)))
                                    //{
                                    
                                    //    if (buff.SkillVNum == 1187)
                                    //    {
                                    //        //session.Character.Session.SendPacket($"mslot 1191 -1");

                                    //        //session.AddBuff(buff, sender, x: x, y: y);
                                    //    }
                                    //    else
                                    //    {
                                    //        //short BuffBefore = (short)(buff.Card.CardId - 1); k mom
                                    //        //session.RemoveBuff(BuffBefore);
                                    //        //session.AddBuff(buff, sender, x: x, y: y);
                                    //        //if(buff.SkillVNum != 1196)
                                    //        //{
                                    //        //    session.Character.Session.SendPacket($"mslot {buff.SkillVNum+1} -1");
                                    //        //}
                                    //    }
                                    //}
...
#warning TODO: Mana Drain
                    ...
                    case BCardType.CardType.BossMonstersSkill:
                        if (SubType == 3)
                        {
                            if (FirstData == 100 && SecondData == 905)
                            {
                                session.Character.AddBuff(new Buff(905, senderLevel), sender);
                                session.Character.Mates.Where(s => s.IsTemporalMate).ToList().ForEach(m =>
                                {
                                    if (!m.IsAlive)
                                    {
                                        m.GenerateRevive();
                                    ...
                                    {
                                        m.AddBuff(new Buff(905, senderLevel), sender);
                                    }
                                });
                            }
                            if (SecondData == 906 && FirstData == 100)
                            {
                                session.Character.AddBuff(new Buff(906, senderLevel), sender);
                                session.Character.AddBuff(new Buff(909, senderLevel), sender);
                                session.Character.Mates.Where(s => s.IsTemporalMate).ToList().ForEach(m =>
                                {
                                    if (m.IsAlive)
                                    {
                                        m.AddBuff(new Buff(906, senderLevel), sender);
                                        m.AddBuff(new Buff(909, senderLevel), sender);
                                    }
                                });
                            }
                        }
                    ...
                    case BCardType.CardType.SummonSkill:
                        ...
                        else if (SubType == 4)
                        {
                            ...
                            sp = session.Character.Inventory.LoadBySlotAndType((byte)EquipmentType.Sp, InventoryType.Wear);
                            if (session.Character.Morph == 42 && FirstData == 0)
                            {
                                session.Character.Morph = 43;
                                session.Character.MapInstance.Broadcast(session.Character.GenerateCMode());
                                session.Character.Session.SendPacket(session.Character.GenerateQuicklist().ToString());
                            }
                            else if (session.Character.Morph == 43 && FirstData != 0)
                            {
                            ...
                            else if (session.Character.Morph == 3720 && FirstData != 0)
                            {
                                session.Character.Morph = 42;
                    ...
                    case BCardType.CardType.InflictSkill:
                        ...
                        switch (SubType)
                        {
                            ...
                            case (byte)AdditionalTypes.InflictSkill.OrcFury / 10:
                                {
                                    ...
                                    user.GetOrcFury((short)FirstData);
                                }
                                break;

                            case (byte)AdditionalTypes.InflictSkill.IncreaseRageOverTime / 10:
                                void IncreaseRageBar()
                                {
                                    if (session.Character != null)
                                    {
                                        session.Character.GetOrcFury(FirstData);
                                    }
                                }
                    ...
                    case BCardType.CardType.FlameLeopard:
                        if(SubType == 3)
                        {
                            if(session.Character.Morph == 42 && FirstData == 100)
                            {
                                session.Character.Morph = 3720;
                                session.Character.MapInstance.Broadcast(session.Character.GenerateCMode());
                                session.Character.Session.SendPacket(session.Character.GenerateQuicklist().ToString());
                                if (session.Character.HasBuff(895))
                                {
                                    session.Character.Speed = 24;
