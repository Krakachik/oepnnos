    ...
    public class DamageHelper
    {
            ...
            ref bool onyxWings, bool attackGreaterDistance = false)
        {
            ...
            boostCategory2 +=
             ...
             if (attacker?.Character != null && attacker.Character.OrcFury > 0)
            {
                boostCategory2 += boostCategory2 * attacker.Character.OrcFury * GetAttackerBenefitingBuffs(CardType.InflictSkill, (byte)AdditionalTypes.InflictSkill.IncreaseAtkPowerOfRage)[0]
                / 1000D;
            }
...
#warning Disabled Critical Rate limit
            ...
            if (attacker?.Character != null)
            {
                if (GetAttackerBenefitingBuffs(CardType.SummonSkill, (byte)AdditionalTypes.SummonSkill.WaterfallDamage) is int[] WaterFall && WaterFall[0] > 0)
                {
                    if (attacker.HasBuff(925))
                    {
                        int amount = attacker.Character.OrcFury * WaterFall[0] / 100;
                        totalDamage += totalDamage * amount / 100;
                        attacker.RemoveBuff(925);
                        attacker.Character.GetOrcFury(attacker.Character.OrcFury == 0 ? amount : -amount);
                    }
                }
            }
