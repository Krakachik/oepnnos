    ...
    public class InventoryPacketHandler : IPacketHandler
    {
        ...
        public void MoveItem(MviPacket mviPacket)
        {
            ...
            if (mviPacket != null)
            {
                ...
                lock (Session.Character.Inventory)
                {
                    ...
                    // 🔥 🔹 **Hier das alte Item entfernen, um UI-Fehler zu vermeiden!**
                    Session.SendPacket(UserInterfaceHelper.Instance.GenerateInventoryRemove(mviPacket.InventoryType, mviPacket.Slot));

                    // 🔹 **Hier das neue Item korrekt hinzufügen**
                    Session.SendPacket(newInventory.GenerateInventoryAdd());

                    // 🔹 **UI-Update senden, um das Inventar sofort zu aktualisieren**
        ...
        private void ChangeSp()
        {
            ...
            if (sp != null)
            {
                ...
                if (sp.ItemVNum == 4582 && sp.Item.Morph == 40 && Session.Character.Class == ClassType.Archer)
                {
                    Session.Character.AddWolf(3134, Session.Character.Level, MateType.Wolf);
                }
