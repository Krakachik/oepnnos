    ...
    public class BattlePacketHandler : IPacketHandler
    {
        ...
        public void UseSkill(UseSkillPacket useSkillPacket)
        {
            ...
            if (Session.Character.CanFight && useSkillPacket != null && !Session.Character.IsSeal)
            {
                ...
                ForceReloadTitleOnMapChange(Session);
        ...
        public void ForceReloadTitleOnMapChange(ClientSession session)
        {
            ...
            session.SendPacket(session.Character.GenerateTitInfo()); // Titel-Paket neu senden!
            session.SendPacket("msg Map gewechselt, Titel neu geladen!");
        }
        ...
        private void PvpHit(HitRequest hitRequest, ClientSession target)
        {
            if (target?.Character.Hp > 0 && hitRequest?.Session.Character.Hp > 0)
            {
                ...
                int reset = target.Character.BattleEntity.GetBuff(CardType.InflictSkill, (byte)AdditionalTypes.InflictSkill.ResetEnemySkill)[0];

                if (reset > 0 && ServerManager.RandomNumber() < reset)
                {
