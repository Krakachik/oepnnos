            ...
            using (StreamReader npcIdStream = new StreamReader(fileNpcId, Encoding.GetEncoding(1252)))
            {
                while ((line = npcIdStream.ReadLine()) != null)
                {
                    ...
                    else if (currentLine.Length > 6 && currentLine[1] == "PREATT")
                    {
                        ...
                        npc.NoticeRange = (byte)short.Parse(currentLine[4]);
                    ...
                    else if (currentLine.Length > 4 && currentLine[1] == "PETINFO")
                    {
                        ...
                        npc.Stars = byte.Parse(currentLine[6]);
                    }
        ...
        public void ImportSkills()
        {
            ...
            using (StreamReader skillIdStream = new StreamReader(fileSkillId, Encoding.GetEncoding(1252)))
            {
                while ((line = skillIdStream.ReadLine()) != null)
                {
                    ...
                    {
                        skill.TargetType = byte.Parse(currentLine[2]);  // Position 2
                        skill.HitType = byte.Parse(currentLine[3]);    // Position 3
                        skill.Range = byte.Parse(currentLine[4]);      // Position 4

                        if (currentLine.Length > 5) // Prüfen, ob Position 5 existiert
                        {
                            // Fehlerprüfung: Ist der Wert eine gültige Zahl im Bereich von 0 bis 255?
                            if (byte.TryParse(currentLine[5], out byte targetRange))
                            {
                                skill.TargetRange = targetRange; // Gültigen Wert zuweisen
                            }
                            else
                            {
                                skill.TargetRange = 0; // Fallback-Wert bei Fehler
                                Logger.Warn($"Invalid TargetRange value for skill {skill.SkillVNum}: {currentLine[5]}");
                            }
                        }
                        else
                        {
                            skill.TargetRange = 0; // Standardwert, wenn Spalte fehlt
                            Logger.Warn($"TargetRange missing for skill {skill.SkillVNum}");
                        }
                    }
                        ...
                        switch (item.ItemType)
                        {
                            ...
                            case ItemType.Jewelery:
                                ...
                                else
                                {
                                    ...
                                    item.MaxCellon = byte.Parse(currentLine[6]);
                                }
                                break;
